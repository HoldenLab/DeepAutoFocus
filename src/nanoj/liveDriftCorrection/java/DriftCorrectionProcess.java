...
public class DriftCorrectionProcess implements Measurements {
    ...
    public float[] PeakFind(FloatProcessor CCmap) {
        int x = CCmap.getWidth()/2 - 15;
        int y = CCmap.getHeight()/2 - 15;
        CCmap.setRoi(x,y, 31, 31);
    ...
    public float[] PeakFind2(FloatProcessor CCmap) {
        ...
        FloatProcessor region = CCmap.crop().convertToFloatProcessor();
        float[] Peak = EstimateShiftAndTilt.getMaxFindByOptimization(region);
        Peak[0] = Peak[0] + peakX
        Peak[1] = Peak[1] + peakY
        return Peak
    }
    ...
    public double CenterHeightFind2(FloatProcessor image, FloatProcessor mask){ // 220131 JE updated 220303
        int x = image.getWidth()/2 - 5;
        int y = image.getHeight()/2 - 5;
        image.setRoi(x,y, 11, 11);
        FloatProcessor region = image.crop().convertToFloatProcessor();
